...
def main(hydra_cfg):
    ...
    if gsz == 1:  # each manipulation task is its own lifelong learning task
        ...
        assert n_manip_tasks % gsz == 0, f'[error] task_group_size does not divide n_tasks'
        datasets = []
        n_demos = []
        n_sequences = []
        for i in range(0, n_manip_tasks, gsz):
            dataset = GroupedTaskDataset(manip_datasets[i : i + gsz], task_embs[i : i + gsz])
            datasets.append(dataset)
            n_demos.extend([x.n_demos for x in dataset.sequence_datasets])
            n_sequences.extend([x.total_num_sequences for x in dataset.sequence_datasets])
        ...
        for k in range(n_manip_tasks):
            ...
            for e in range(
                ...
                if e % cfg.eval.eval_every == 0:
                    result_summary[f'k{k}_e{e // cfg.eval.eval_every}'] = [[] for _ in range(cfg.eval.n_eval)]
    ...
    if cfg.lifelong.algo == 'Multitask':
        ...
        if cfg.eval.eval:
            ...
            if cfg.use_wandb:
                wandb.run.summary['success_confusion_matrix'] = result_summary['S_conf_mat']
                wandb.run.summary['loss_confusion_matrix'] = result_summary['L_conf_mat']
