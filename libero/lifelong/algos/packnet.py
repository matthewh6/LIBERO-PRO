...
class PackNet(Sequential):
    ...
    def pruning_mask(self, weights, previous_mask, layer_idx):
        ...
        tensor = weights[previous_mask.eq(self.current_task + 1)]  # current_task starts from 0, so we add 1
        ...
        remove_mask = weights.abs().le(cutoff_value) * previous_mask.eq(self.current_task + 1)
    ...
    def prune(self):
        ...
        """
        self.current_masks = {}
        print('[info] pruning each layer by removing %.2f%% of values' % (100 * self.cfg.lifelong.prune_perc))
    ...
    def end_task(self, dataset, task_id, benchmark):
        ...
        if self.cfg.lifelong.post_prune_epochs:
            ...
            for epoch in range(0, self.cfg.lifelong.post_prune_epochs + 1):
                ...
                print(f'[info] Post epoch: {epoch:3d} | train loss: {training_loss:5.2f} | time: {(t1 - t0) / 60:4.2f}')
                time.sleep(0.1)
    ...
    def get_eval_algo(self, task_id):
        ...
        eval_algo = safe_device(
            eval(self.cfg.lifelong.algo)(eval(self.cfg.benchmark_name)().n_tasks, self.cfg),
            self.cfg.device,
        )
        model_state_dict, _, _ = torch_load_model(os.path.join(self.experiment_dir, 'tmp_model.pth'))
