        ...
        if self.name in standard_10_task_suites and n_tasks_actual == 10:
            ...
            if 0 <= self.task_order_index < len(task_orders):
                ...
                try:
                    ...
                    print(
                        f"[error] Task order permutation {order} is invalid for the {n_tasks_actual} tasks found in benchmark '{self.name}'. Using default order."
                    )
                    self.tasks = tasks
            else:
                # Fallback if task_order_index is out of range
                print(
                    f"[warning] task_order_index {self.task_order_index} is out of range for available orders [0, {len(task_orders) - 1}]. Using default task order for benchmark '{self.name}'."
                )
                self.tasks = tasks
        else:
