...
class BDDLBaseDomain(SingleArmEnv):
    ...
    def _assert_problem_name(self):
        ...
        assert self.parsed_problem['problem_name'] == self.__class__.__name__.lower(), 'Problem name mismatched'
    ...
    def _generate_object_state_wrapper(self, skip_object_names=['main_table', 'floor', 'countertop', 'coffee_table']):
        object_states_dict = {}
    ...
    def _load_model(self):
        ...
        if self._arena_type == 'table':
            ...
            xpos = self.robots[0].robot_model.base_xpos_offset['kitchen_table'](self.kitchen_table_full_size[0])
            self.robots[0].robot_model.set_base_xpos(xpos)
            ...
            xpos = self.robots[0].robot_model.base_xpos_offset['coffee_table'](self.coffee_table_full_size[0])
            self.robots[0].robot_model.set_base_xpos(xpos)
            ...
            xpos = self.robots[0].robot_model.base_xpos_offset['living_room_table'](self.living_room_table_full_size[0])
            self.robots[0].robot_model.set_base_xpos(xpos)
            ...
            xpos = self.robots[0].robot_model.base_xpos_offset['study_table'](self.study_table_full_size[0])
            self.robots[0].robot_model.set_base_xpos(xpos)
    ...
    def _setup_placement_initializer(self, mujoco_arena):
        ...
        self.conditional_placement_initializer = SiteSequentialCompositeSampler(name='ConditionalSiteSampler')
        self.conditional_placement_on_objects_initializer = SequentialCompositeSampler(name='ConditionalObjectSampler')
    ...
    def _add_placement_initializer(self):
        ...
        for state in initial_state:
            ...
            if state[0] in ['open', 'close']:
                ...
                if state[1] in self.object_states_dict and hasattr(self.object_states_dict[state[1]], 'set_joint'):
                ...
                if state[1] in self.object_states_dict and hasattr(self.object_states_dict[state[1]], 'set_joint'):
                    obj = self.get_object(state[1])
                    if state[0] == 'turnon':
                        joint_ranges = obj.object_properties['articulation']['default_turnon_ranges']
                    else:
                        joint_ranges = obj.object_properties['articulation']['default_turnoff_ranges']
        ...
        for state in conditioned_initial_place_state_on_objects:
            ...
            self.conditional_placement_on_objects_initializer.append_sampler(sampler, {'reference': other_object_name})
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            object_placements = self.conditional_placement_initializer.sample(self.sim, object_placements)
            object_placements = self.conditional_placement_on_objects_initializer.sample(object_placements)
